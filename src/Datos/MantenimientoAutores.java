 ...
 * @author user
 */
public class MantenimientoAutores {
     BDConexion con;
     ResultSet rs;
    ...
    public void RegistroProv(Autores prov ){           
            String insertar = "INSERT INTO tbautor(claveautor,nombre,rfc,telefono,direccion,correo)values(?,?,?,?,?,?)";
        try {
            con = new BDConexion();
    ...
    public void Eliminar(Autores prov) {
        try {
            String sql = "DELETE FROM tbautor WHERE claveautor=" + prov.getClaveautor();
            con = new BDConexion();
    ...
    public JTable BuscarAutorTabla(JTable tablaProveedor){
        try{
            String sql;
            con=new BDConexion();
            sentenciaSQL=con.conectarse().createStatement();
    ...
    return cbproveedor;
     ...
     public void Actualizar(Autores prov) throws SQLException {
        String sql;
        sql = "UPDATE tbautor SET claveautor=?,nombre=?,rfc=?,telefono=?,direccion=?,correo=? WHERE claveautor=?";
        try {
            con = new BDConexion();
